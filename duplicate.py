...
class DuplicateChecker:
    ...
    def _get_conn(self):
        if not hasattr(self._local, "conn"):
            # Добавлен timeout=10 и WAL-режим для предотвращения "database is locked"
    ...
    def _init_db(self):
        ...
        """)
        try: cursor.execute("ALTER TABLE seen_codes ADD COLUMN operator TEXT")
        except: pass
        try: cursor.execute("ALTER TABLE seen_codes ADD COLUMN workplace TEXT")
        except: pass
        try: cursor.execute("ALTER TABLE seen_codes ADD COLUMN sscc TEXT")
        except: pass
        ...
        try: cursor.execute("CREATE INDEX IF NOT EXISTS idx_seen_codes_sscc ON seen_codes(sscc)")
        except: pass
        conn.commit()
        ...
        with conn:
            ...
            try:
                ...
                if req.get('key') != self.access_key:
                    res = {"status": "error", "message": "Security Error: Invalid Key"}
                else:
    ...
    def _handle_network_request(self, req):
        try:
            if req['type'] == 'ping': return {"status": "ok"}
    ...
    def check(self, code, operator=None, workplace=None):
        if self.is_server:
            ...
            res = self.network_request({"type": "check_unit", "code": code, "operator": operator, "workplace": workplace})
    ...
    def check_sscc(self, code):
        if self.is_server: self.check_sscc_local(code)
        else:
