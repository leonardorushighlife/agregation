...
class App:
    ...
    def admin_panel(self):
        ...
        row += 1
        tk.Label(scrollable_frame, text="Proxy Enabled", bg="#f0f0f0").grid(row=row, column=0, sticky="w", padx=10)
        proxy_enabled_v = tk.BooleanVar(value=self.config.get("proxy_enabled", False))
        tk.Checkbutton(scrollable_frame, variable=proxy_enabled_v, bg="#f0f0f0").grid(row=row, column=1, sticky="w")

        row += 1
        tk.Label(scrollable_frame, text="Proxy Type", bg="#f0f0f0").grid(row=row, column=0, sticky="w", padx=10)
        ...
        row += 1
        tk.Label(scrollable_frame, text="Host", bg="#f0f0f0").grid(row=row, column=0, sticky="w", padx=10)
        ...
        row += 1
        tk.Label(scrollable_frame, text="Port", bg="#f0f0f0").grid(row=row, column=0, sticky="w", padx=10)
        ...
        row += 1
        tk.Label(scrollable_frame, text="User", bg="#f0f0f0").grid(row=row, column=0, sticky="w", padx=10)
        ...
        row += 1
        tk.Label(scrollable_frame, text="Password", bg="#f0f0f0").grid(row=row, column=0, sticky="w", padx=10)
    ...
    def toggle_mode_fields(self):
        ...
        elif mode in ["unit", "warehouse_unit_acc", "set"]:
    ...
    def start_shift(self):
        ...
        elif self.agg_mode in ["unit", "warehouse_unit_acc", "set"]:
            if not self.config.get("box_size_fixed", True):
                try:
    ...
    def end_shift(self):
        ...
        if self.state.in_box != 0:
            err_msg = t["need_close_box"] if self.state.mode != "set" else t["need_close_set"]
            self._msg_box(messagebox.showwarning, t["error"], err_msg); return
    ...
    def on_scan_unit(self, raw):
        ...
        if self.state.wait_sscc:
            if self.state.mode != "set" and not raw.startswith("00"):
            ...
            try:
                ...
                if self.state.mode != "set" and self.config["gtin_enabled"] and parsed["gtin"] != self.config["gtin"]:
