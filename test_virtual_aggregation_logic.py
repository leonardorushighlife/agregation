...
class TestVirtualAggregationLogic(unittest.TestCase):
    ...
    def test_read_codes_error_handling(self):
        # Проверяем, что при ошибке UnicodeError бросается ValueError
        dummy_path = os.path.join(self.test_dir, "dummy.txt")
        with open(dummy_path, "w", encoding="utf-8") as f:
            f.write("test")

        with patch("builtins.open") as mock_open:
            mock_open.side_effect = UnicodeError("Mocked Unicode error")
            with self.assertRaises(ValueError):
                read_codes(dummy_path)
